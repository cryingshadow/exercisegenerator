...
public interface FlowNetworkAlgorithm extends AlgorithmImplementation<FlowNetworkProblem, FlowNetworkResult> {

    public static final int DEFAULT_SOURCE_SINK_ROOT = 7;

    public static final String RESIDUAL_GRAPH_NAME = "Restnetzwerk";
            ...
            if (prevMaxYPos == prevMinYPos) {
                ...
                if (minDiagonal) {
                    if (maxDiagonal) {
                        ...
                        if (
                            FlowNetworkAlgorithm.enoughVerticesForLevel(
                                remainingVertices,
                                prevMinYPos - 1,
                                prevMaxYPos + 1,
                                xPos
                    ...
                    if (
                        ...
                        FlowNetworkAlgorithm.enoughVerticesForLevel(remainingVertices, prevMinYPos, prevMaxYPos, xPos)
                    ) {
                        options.add(keepMin + keepMax);
                        options.add(keepMin + keepMax);
                    }
                        ...
                        Optional.of(
                            new FlowAndCapacity(
                                ...
                                GraphAlgorithm.randomEdgeValue(
                                    prevXPos == 0 ?
                                        FlowNetworkAlgorithm.DEFAULT_SOURCE_SINK_ROOT :
        ...
        for (final Vertex<String> otherVertex : existing) {
            graph.addEdge(
                ...
                Optional.of(
                    new FlowAndCapacity(
                        0,
                        GraphAlgorithm.randomEdgeValue(FlowNetworkAlgorithm.DEFAULT_SOURCE_SINK_ROOT)
                    )
                ),
                sink
            );
    ...
    private static FlowNetworkWithHighlights computeGraphWithFlow(
        ...
        final Integer min = FlowNetworkAlgorithm.computeMinEdge(graph, flowPath);
        ...
        return new FlowNetworkWithHighlights(graph, toHighlight);
    }
    ...
    private static FlowNetworkConfiguration parseOrGenerateConfiguration(
        ...
        return new FlowNetworkConfiguration(multiplier, twoColumns);
    }
    ...
    private static void printFordFulkersonDoubleStep(
        ...
        final FlowNetworkConfiguration configuration,
        ...
        LaTeXUtils.printSamePageBeginning(
            ...
            configuration.twoColumns() ? LaTeXUtils.TWO_COL_WIDTH : LaTeXUtils.LINE_WIDTH,
            Optional.of(FlowNetworkAlgorithm.RESIDUAL_GRAPH_NAME),
            writer
        );
        ...
        if (solution) {
            layout =
                layout.highlight(
                    FlowNetworkAlgorithm.filterHighLights(step.residualHighlights())
                    .stream()
    ...
    private static void printStatisticsAsComment(
        ...
        final Pair<Integer, Integer> numFlowValues =
            FlowNetworkAlgorithm.computeNumberOfNewAndCopiedFlowValues(solution);
        final int numResidualEdges = FlowNetworkAlgorithm.computeNumberOfResidualEdges(solution);
    ...
    private static Set<Vertex<String>> toSourcePartition(
        final Graph<String, FlowAndCapacity> flowNetwork,
        final Vertex<String> source
    ) {
        final Set<Vertex<String>> result = new TreeSet<Vertex<String>>();
        result.add(source);
        ...
        while (!queue.isEmpty()) {
            ...
            used.add(vertex);
            result.add(vertex);
            for (final Edge<FlowAndCapacity, String> edge : flowNetwork.getAdjacencySet(vertex)) {
                if (edge.label().get().flow() < edge.label().get().capacity()) {
                    queue.add(edge.to());
                }
            }
        }
    ...
    @Override
    public default FlowNetworkResult apply(final FlowNetworkProblem problem) {
        ...
        Graph<String, Integer> residualGraph = FlowNetworkAlgorithm.computeResidualGraph(graph);
        List<Vertex<String>> path = FlowNetworkAlgorithm.selectAugmentingPath(residualGraph, source, sink);
        ...
        result.add(
            new FordFulkersonDoubleStep(
                ...
                FlowNetworkAlgorithm.toEdges(residualGraph, path)
            )
        );
        while (path != null) {
            final FlowNetworkWithHighlights graphWithAddedFlow = FlowNetworkAlgorithm.computeGraphWithFlow(graph, path);
            graph = graphWithAddedFlow.graph();
            residualGraph = FlowNetworkAlgorithm.computeResidualGraph(graph);
            path = FlowNetworkAlgorithm.selectAugmentingPath(residualGraph, source, sink);
            result.add(
                new FordFulkersonDoubleStep(
        ...
        return new FlowNetworkResult(
            result,
            FlowNetworkAlgorithm.toSourcePartition(result.getLast().flowNetworkWithLayout().graph(), source)
        );
    }

    @Override
    public default FlowNetworkProblem generateProblem(final Parameters<Flag> options) {
        ...
        final GraphWithLayout<String, FlowAndCapacity, Integer> graphWithLayout =
            FlowNetworkAlgorithm.createRandomFlowNetwork(numOfInnerVertices);
    ...
    @Override
    public default String[] generateTestParameters() {
    ...
    @Override
    public default List<FlowNetworkProblem> parseProblems(
        final BufferedReader reader,
        final Parameters<Flag> options
    ) throws IOException {
        final GraphWithLayout<String, FlowAndCapacity, Integer> graphWithLayout =
            options.containsKey(Flag.VARIANT) && options.getAsInt(Flag.VARIANT) == 1 ?
                FlowNetworkAlgorithm.parsePositionedGraph(reader) :
    ...
    @Override
    public default void printBeforeSingleProblemInstance(
        final FlowNetworkProblem problem,
    ...
    @Override
    public default void printProblemInstance(
        final FlowNetworkProblem problem,
        final FlowNetworkResult solution,
        final Parameters<Flag> options,
        final BufferedWriter writer
        ...
        final FlowNetworkConfiguration configuration =
            FlowNetworkAlgorithm.parseOrGenerateConfiguration(problem, options);
    ...
    @Override
    public default void printSolutionInstance(
        final FlowNetworkProblem problem,
        final FlowNetworkResult solution,
        final Parameters<Flag> options,
        final BufferedWriter writer
        ...
        for (final FordFulkersonDoubleStep step : solution.steps()) {
            ...
            if (first) {
                first = false;
                FlowNetworkAlgorithm.printFordFulkersonDoubleStep(
                    stepNumber,
                    step,
                    true,
                    true,
        ...
        FlowNetworkAlgorithm.printStatisticsAsComment(solution.steps(), writer);
    }

    @Override
    public default void printSolutionSpace(
        final FlowNetworkProblem problem,
        final FlowNetworkResult solution,
        final Parameters<Flag> options,
        final BufferedWriter writer
