...
public class IntRBTree {
     ...
     */
    public RBNode minimum(RBNode rootOfSubtree) {
    ...
    public void remove(RBNode node, BufferedWriter writer, boolean write) throws IOException {
     ...
     */
    private RBNode adjust(boolean isLeft, RBNode node, BufferedWriter writer, boolean write) throws IOException {
        RBNode current = node;
        RBNode uncle = current.getFather().getFather().getSucc(!isLeft);
     ...
     * @param write A boolean, which is true if the Red-Black-Tree has to be printed after each rotation.
     */
    private void balanceAfterInsert(RBNode node, BufferedWriter writer, boolean write) throws IOException {
        RBNode current = node;
     ...
     * @param writer The writer to send the output to.
     */
    private void balanceAfterRemove(RBNode old, RBNode node, BufferedWriter writer, boolean write) throws IOException {
        if (!old.isBlack()) {
            return;
        }
        if (!node.isBlack()) {
     ...
     * @param write A Boolean, which is true if the Red-Black-Tree has to be printed after each rotation.
     */
    private void rbInsert(int value, BufferedWriter writer, boolean write) throws IOException {
        RBNode node = this.insert(value);
        if (node == this.root) {
     ...
     * @param writer The writer to send the output to.
     */
    private RBNode rmAdjust(boolean isLeft, RBNode node, BufferedWriter writer, boolean write) throws IOException {
     ...
     */
    private RBNode rotate(boolean isLeft, RBNode node, BufferedWriter writer, boolean write, String prefix)
    throws IOException {
        RBNode succ = node.getSucc(!isLeft);
        // Baum B verschieben
    ...
    private void swap(RBNode oldNode, RBNode newNode) {
     ...
     */
    private String toString(RBNode node) {
    ...
    private static class RBNode {
        ...
        public RBNode getSucc(boolean isLeft) {
            if (isLeft) {
                return this.left;
            } else {
                return this.right;
        ...
        public void setFather(RBNode newFather) {
        ...
        public void setSucc(boolean isLeft, RBNode node) {
            if (isLeft) {
                this.left = node;
            } else {
                this.right = node;
